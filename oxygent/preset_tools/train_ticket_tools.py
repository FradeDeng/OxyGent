...
def get_stations() -> Dict[str, StationData]:
    """
    Fetch and parse all railway station data from the 12306 website.

    Returns:
        A dictionary of station data with station_code as keys

    Raises:
        Exception: If unable to fetch or parse the station data
    """
    ...
    try:
        ...
        raise Exception('[Error]: Get the main page HTML failed.')

    # Extract the station name JS file path using regex
    match = re.search(r'(/script/core/common/station_name.+?\.js)', html)
    if match is None:
        raise Exception('[Error]: Get station name js file failed.')

    # Get the station name JS file
    ...
    # Extract the raw data from the JS file
...
def parse_stations_data(raw_data: str) -> Dict[str, StationData]:
    """
    Parse the raw station data string into a structured dictionary.

    Args:
        raw_data: A pipe-delimited string containing station information

    Returns:
        A dictionary with station_code as keys and station data as values
    """
    # Keys used to parse station data
    station_data_keys = [
        'station_id',
        'station_name',
        'station_code',
    ...
    # Group data into chunks of 10 items (each representing one station)
    for i in range(len(data_array) // 10):
        data_list.append(data_array[i * 10:(i * 10) + 10])

    # Process each station data group
    for group in data_list:
        station: Dict[str, str] = {}

        # Map each value to its corresponding key
        for index, key in enumerate(station_data_keys):
            station[key] = group[index]

        # Skip stations without a station_code
        if not station['station_code']:
            continue

        # Add the station to the result dictionary using station_code as key
...
def get_cookie():
    ...
    try:
        ...
        print(f"Error making 12306 request: {error}")
        return None
...
def parse_cookies(cookies):
    ...
    for cookie in cookies:
        ...
        # 分割键和值
        parts = key_value_part.split('=', 1)  # 最多分割一次，以处理值中可能包含=的情况
        if len(parts) == 2:
            key, value = parts
            # 存入字典
            if key and value:
